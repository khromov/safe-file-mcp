...
describe('CLI with Commander', () => {
  ...
  describe('Main command', () => {
    ...
    it('should accept --mini flag', async () => {
      ...
      const serverProcess = exec(`PORT=${port} node ${CLI_PATH} --mini`, (error, stdout, stderr) => {
    ...
    it('should accept --full flag', async () => {
      ...
      const serverProcess = exec(`PORT=${port} node ${CLI_PATH} --full`, (error, stdout, stderr) => {
        // Process will be killed, so we don't check here
      });
  ...
  describe('ls subcommand', () => {
    ...
    it('should sort by path when specified', async () => {
      const { stdout } = await execAsync(`node ${CLI_PATH} ls --directory ${TEST_DIR} --sort-by path`);
